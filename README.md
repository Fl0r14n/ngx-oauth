...
> Zoneless / signal-based. No `OAuthService`, no RxJS. Public API is a set of `InjectionToken`s exposing signals and
> async functions.
...
### Login component (optional or provide your own)
...
const config = {
    ...
    logoutRedirectUri: `${location.origin}/`, // optional
    state: crypto.randomUUID(), // optional
...
<oauth-login [config]="config" [i18n]="{ username: 'Email', submit: 'Entrar' }"/>
```

### OAuth callback route

For redirect-based flows (authorization code / implicit), the IdP returns to a callback URL. Wire a route guard that
calls `oauthCallback`:
...
**SSR**: the callback route reads `window.location` (`hash`, `search`) and writes to `localStorage`, so it must render
on the client. Mark it `RenderMode.Client` in `app.routes.server.ts`:
...
OpenID Connect logout with post-logout redirect and `id_token_hint`:

```typescript
const oauth = inject(OAUTH)
await oauth.logout(`${location.origin}/logged-out`, 'optional-state')
```

Requires `logoutPath` in and optional `logoutRedirectUri` in config.
...
export class MyComponent {
    ...
    login() {
        return this.oauth.login({username: 'u', password: 'p'})
    }
...
A reactive `effect` watches the token and refreshes automatically when expired (if `refresh_token` is present and
`tokenPath` is configured). The previous `refresh_token` is preserved when the IdP omits it in the refresh response.
`OAUTH_FETCH` deduplicates concurrent refreshes (single-flight).

### Introspection

Check if a token is active via the introspection endpoint:

```typescript
import {OAUTH_INTROSPECT} from 'ngx-oauth'

const introspect = inject(OAUTH_INTROSPECT)
const info = await introspect(token(), config())
// { active: true, scope: 'read write', exp: 1715628400, ... }
```

Requires `introspectionPath` in config.
