...
export const OAuthInterceptor: HttpInterceptorFn = (req: HttpRequest<unknown>, next: HttpHandlerFn) => {
  const authConfig = inject(OAuthConfig);
  const tokenService = inject(OAuthTokenService);
  const isPathExcepted = (req: HttpRequest<unknown>) => {
